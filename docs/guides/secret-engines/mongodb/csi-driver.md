...
>Note: Yaml files used in this tutorial stored in [docs/examples/csi-driver/database/mongodb](/docs/0.1.0/examples/csi-driver/database/mongodb) folder in github repository [kubevault/docs](https://github.com/kubevault/docs)
...
There are two ways to configure `Vault server`. You can use either `Vault Operator` or  `existing Vault server` to manage MongoDB credentials. Here we describe both ways,how can we configure `Vault` to store the credentials.
...
<div class="tab-content" id="conceptsTabContent">
  <div class="tab-pane fade show active" id="operator" role="tabpanel" aria-labelledby="operator-tab">
   Follow <a href= "/docs/0.1.0/guides/secret-engines/mongodb/overview"> this</a> tutorial to manage MongoDB credentials with `Vault operator`.
