---
title: Mount PostgreSQL credentials using CSI Driver
menu:
  docs_{{ .version }}:
...
# Mount PostgreSQL credentials using CSI Driver
...
## Enable and Configure PostgreSQL Database Secrets Engine

The following steps are required to enable and configure the PostgreSQL database secrets engine in the Vault server.
...
Let's enable and configure the PostgreSQL database secret engine by deploying the following `SecretEngine` yaml:
...
spec:
  secret:
    name: postgres-user-cred # k8s secret name
  clientConfig:
    service:
      name: postgres
      scheme: postgresql
...
Create database role using the following `PostgresRole` yaml:
...
You can also use [Vault CLI](https://www.vaultproject.io/docs/commands/) to [enable and configure](https://www.vaultproject.io/docs/secrets/databases/postgresql.html#setup) the PostgreSQL secret engine.
...
```console
$ vault secrets enable -path=my-postgres-se database
Success! Enabled the database secrets engine at: my-postgres-se/
```

- **Configure Secret Engine:** Configure Vault with the proper plugin and connection information by running:

```console
$ vault write my-postgres-se/config/k8s.-.demo.postgres-app \
    plugin_name=postgresql-database-plugin \
    allowed_roles="*" \
    connection_url="postgresql://{{username}}:{{password}}@159.203.114.170:30595/postgresdb?sslmode=disable" \
    username="postgresadmin" \
    password="admin123"
Success! Data written to: my-postgres-se/config/k8s.-.demo.postgres-app
```
...
```console
$ vault write my-postgres-se/roles/k8s.-.demo.psql-role \
    db_name=k8s.-.demo.postgres-app \
...
```console
$ kubectl apply -f examples/guides/secret-engines/postgres/storageClass.yaml
storageclass.storage.k8s.io/vault-psql-storage created
```
...
```console
$ kubectl apply -f examples/guides/secret-engines/postgres/pvc.yaml
persistentvolumeclaim/csi-pvc-psql created
```
...
spec:
  vaultRef:
    name: vault
  # Here, postgres secret engine is enabled at "my-postgres-se".
...
```console
$ kubectl apply -f examples/guides/secret-engines/postgres/vaultPolicy.yaml
vaultpolicy.policy.kubevault.com/psql-se-policy created

$ kubectl apply -f examples/guides/secret-engines/postgres/vaultPolicyBinding.yaml
vaultpolicybinding.policy.kubevault.com/psql-se-role created
```
...
metadata:
  ...
  namespace: trial
  annotations:
    secrets.csi.kubevault.com/vault-role: k8s.-.demo.psql-se-role
```

```console
$ kubectl apply -f examples/guides/secret-engines/postgres/podServiceAccount.yaml
  ...
  volumes:
    - name: my-vault-volume
      persistentVolumeClaim:
        claimName: csi-pvc-psql
```

```console
$ kubectl apply -f examples/guides/secret-engines/postgres/pod.yaml
pod/mypod created
```
