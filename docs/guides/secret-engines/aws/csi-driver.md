...
We need to configure following things in this step to retrieve `AWS` IAM secrets from `Vault server` into Kubernetes pod.
...
There are two ways to configure `Vault server`. You can use either `Vault Operator` or  `existing Vault server` to manage `AWS IAM` secrets. Here we describe both ways,how can we configure `Vault` to store the credentials.
