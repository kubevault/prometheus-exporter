...
Now we are going to create an [AppBinding](/docs/concepts/appbinding-crds/appbinding.md) that contains connection and credential information of the Vault that is deployed in `demo-cluster-1`. In this AppBinding, we are going to use [token auth](https://www.vaultproject.io/docs/auth/token.html#token-auth-method). Guides to Vault authentication using AppBinding can be found [here](/docs/concepts/appbinding-crds/vault-authentication-using-appbinding.md).
